...
if (!isset($_GET['id_athlete'])) {
    ...
    if ($id_athlete === false) {
        ...
        try {
            // Préparez la requête SQL pour supprimer le athlete
            ...
            // Message de succès
            $_SESSION['success'] = "Le athlete a été supprimé avec succès.";

            // Redirigez vers la page précédente après la suppression
            header('Location: manage-athletes.php');
            exit();
        } catch (PDOException $e) {
            $_SESSION['error'] = "Erreur lors de la suppression du athlete : " . htmlspecialchars($e->getMessage(), ENT_QUOTES, 'UTF-8');
            header('Location: manage-athletes.php');
            exit();
        }
    }
